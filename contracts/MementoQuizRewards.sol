...
contract MementoQuizRewards is ERC20, Ownable, Pausable {
     ...
     * @param recipient Address to receive the tokens (allows gasless claims via relayers)
    ...
    function claimReward(
        ...
        // NOTE: recipient is included in the hash to prevent unauthorized claims
        bytes32 messageHash = keccak256(abi.encodePacked(
            recipient,  // Changed from msg.sender to recipient
            sessionId,
            score,
            deadline,
            block.chainid,
        ...
        // Mint reward tokens to recipient (not msg.sender!)
        _mint(recipient, REWARD_AMOUNT);
        
        emit RewardClaimed(recipient, sessionId, score, REWARD_AMOUNT);
    }
